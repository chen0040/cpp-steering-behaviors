...
GLState_BSpline::GLState_BSpline()
: GLState()
, m_pCommands(NULL)
{
	SetTitleTexture("images\\button_normal_demo_BSpline.jpg");
	m_state_name="State_BSpline";
}
...
GLState_BSpline* GLState_BSpline::Instance()
{
	static GLState_BSpline theInstance;
	return &theInstance;
}

void GLState_BSpline::Entered(GameWorld* pWorld)
{
	glConsole << "\nState - BSpline\n";
	glConsole << "click mouse on the plane to generates points for BSpline curve\n";
...
void GLState_BSpline::SetupUI(GameWorld* pWorld)
{
	if(m_pCommands == NULL)
	{
...
void GLState_BSpline::Exited(GameWorld *pWorld)
{
	pWorld->RemoveAgents();
}

void GLState_BSpline::Update(GameWorld *pWorld, const long &lElapsedTicks)
{
	pWorld->UpdateAgents(lElapsedTicks);
}

void GLState_BSpline::Render(GameWorld* pWorld)
{
...
void GLState_BSpline::MouseButtonDown(GameWorld* pWorld)
{
...
void GLState_BSpline::MouseMoved(GameWorld* pWorld)
{
	GLMouse& game_mouse=pWorld->GetMouse();
	int iButton=game_mouse.GetMouseButton();
	int iX=game_mouse.GetMouseX();
